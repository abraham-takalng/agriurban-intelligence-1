...
const LOGO_URL = "https://storage.googleapis.com/dala-prod-public-storage/attachments/820e4cfb-b67b-4e0d-acfe-8a02a845cd07/1774729697680_Agriurban_Intelligence_Hexagonal_Logo.svg";
    ...
    <div className="min-h-screen bg-[#050505] text-neutral-100 font-sans selection:bg-cyan-500/30 selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
          ...
          >
            <div className="w-12 h-12 transition-transform group-hover:scale-110 duration-300 flex items-center justify-center">
              <img src={LOGO_URL} alt="AgriUrban Intelligence Logo" className="w-full h-full object-contain filter drop-shadow-[0_0_8px_rgba(79,70,229,0.3)]" />
            </div>
            <div>
          ...
          <div className="hidden xl:flex items-center gap-1">
            ...
            <button 
              ...
              className="ml-4 px-5 py-2 bg-neutral-800 border border-white/10 rounded-full text-xs font-semibold text-white hover:bg-neutral-700 transition-all active:scale-95"
            >
              Sign Out
            </button>
            <button 
              onClick={() => changePage('map')}
              className="ml-2 px-5 py-2 bg-gradient-to-r from-cyan-600 to-emerald-600 rounded-full text-xs font-semibold text-white shadow-lg shadow-cyan-500/20 hover:opacity-90 transition-all active:scale-95"
            >
              Launch Platform
            </button>
          </div>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-2 pb-10">
              ...
              <button
                  onClick={handleSignOut}
                  className="flex items-center gap-4 p-4 rounded-2xl text-lg font-medium text-red-400 mt-4"
                >
                  <X className="w-6 h-6" />
                  Sign Out
                </button>
      ...
      <main className="min-h-screen">
        <AnimatePresence mode="wait">
          ...
          >
            {currentPage === 'home' && <LandingPage onNavigate={changePage} />}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            ...
            <div>
              ...
              <ul className="space-y-4">
                <li><button onClick={() => changePage('map')} className="text-neutral-400 hover:text-white transition-colors">Map Explorer</button></li>
                <li><button onClick={() => changePage('disaster')} className="text-neutral-400 hover:text-white transition-colors">Disaster Intelligence</button></li>
                <li><button onClick={() => changePage('preparedness')} className="text-neutral-400 hover:text-white transition-colors">Preparedness Guides</button></li>
                <li><button onClick={() => changePage('ai')} className="text-neutral-400 hover:text-white transition-colors">AI Analysis</button></li>
              </ul>
            </div>
  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        <Route 
          path="/welcome" 
          element={!session ? <WelcomePage /> : <Navigate to="/dashboard" />} 
        />
        <Route 
          path="/signin" 
        ...
        <Route 
          path="/dashboard/*" 
          element={session ? <MainApp /> : <Navigate to="/welcome" />} 
        />
        <Route 
          path="/" 
          element={<Navigate to={session ? "/dashboard" : "/welcome"} />} 
        />
        {/* Catch-all */}
